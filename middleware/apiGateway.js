 ...
 *   3. Resolve endpoint via JOIN (match reqPath against endpoint_path)
 *   4. Daily quota reset
 *   5. Check quota remaining
 *   6. Deduct quota_used
 *   7. Proxy request to target_url
 */
async function apiGateway(req, res, next) {
  try {
    ...
    // ── 3. Resolve Endpoint ──────────────────────────────────────────────────
    // Gateway mounted at /api, so req.path = "/todos", req.baseUrl = "/api"
    // endpoint_path stored as "/api/todos" — reconstruct to match
